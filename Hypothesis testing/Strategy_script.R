...
specialism <- read.csv("../Data/UKBMS_data/UKBMS_UKspecieslist.csv", header=TRUE)
specialism <- specialism[,c("BMSCODE", "STRATEGY")]
colnames(specialism) <- c("BMSCODE", "specialism")

pair_attr <- merge(pair_attr, specialism, by.x="spp", by.y="BMSCODE", all.x=TRUE)
...
spec_model_ukbms2 <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + scale(renk_hab_sim) + scale(mid.year)*specialism + 
                            scale(winter_rain) + scale(autumn_rain) + scale(spring_rain) + scale(summer_rain) + 
                            scale(winter_temp) + scale(autumn_temp) + scale(spring_temp) + scale(summer_temp) + 
...
results_table <- merge(results_table, specialism, by.x="i", by.y="BMSCODE", all.x=TRUE)
...
results_table$Specialism <- revalue(results_table$Specialism, c("Habitat specialist"="Specialist"))
results_table$Specialism <- revalue(results_table$Specialism, c("Wider countryside sp"="Generalist"))
