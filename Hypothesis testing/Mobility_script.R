...
butterfly_mob <- read.csv("../Data/UKBMS_data/species.traits.full.table.csv", header = TRUE) # mobility trait data for each species
...
pair_attr <- merge(pair_attr, butterfly_mob, by.x="spp", by.y="species", all.x=TRUE)
...
mobility_model <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + scale(renk_hab_sim) + mid.year + scale(winter_rain) + scale(autumn_rain) + 
                         ...
                         scale(summer_temp) + scale(mobility_wil) + (1|spp) + (1|pair.id), data=pair_attr)
...
mobility_model6 <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + scale(renk_hab_sim) + scale(winter_rain) + scale(autumn_rain)
                        + scale(spring_rain) + scale(summer_rain) + scale(winter_temp) + scale(autumn_temp) + scale(spring_temp)
...
library(ggeffects)
dat <- ggpredict(mobility_model6, terms = c("mid.year", "mobility_wil"))
...
ggplot(mapping=aes(x=Mobility, y=slope, ymin=slope-SE, ymax=slope+SE)) +
  ...
  labs(x="Mobility score", y="Change in population synchrony 2000-2012") +
...
## log dispersal before standardising
#pair_attr_bbs$Breeding_AM <- log(pair_attr_bbs$Breeding_AM)
#pair_attr_bbs4[39] <- lapply(pair_attr_bbs4[39], function(pair_attr_bbs4) c(scale(pair_attr_bbs4, center = TRUE, scale = TRUE))) 

###### run model with mobility and year interaction
dispersal_model_bbs <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + scale(renk_hab_sim) + scale(winter_rain) + 
                               scale(spring_rain) + scale(autumn_rain) + scale(mid.year)*scale(Breeding_AM) + (1|pair.id) + (1|spp), data = pair_attr_bbs4)
...
results_table_dispersal_bbs <- data.frame(summary(dispersal_model_bbs)$coefficients[,1:5])
write.csv(results_table_dispersal_bbs, file = "../Results/Model_outputs/BBS/change_mob_bbs.csv", row.names=TRUE)

library(DHARMa)
testDispersion(dispersal_model_bbs1) ## possible underdispersion (if red line is to the left = underdispersion)
simulationOutput <- simulateResiduals(fittedModel = dispersal_model_bbs1, plot = F)
plot(simulationOutput) ## no assumptions violated!
x <- outliers(simulationOutput)

## check for outliers
cooksd <- cooks.distance(dispersal_model_bbs1)
# Plot the Cook's Distance using the traditional 4/n criterion
sample_size <- nrow(pair_attr_bbs)
plot(cooksd, pch="*", cex=2, main="Influential Obs by Cooks distance")  # plot cook's distance
abline(h = 4*mean(cooksd), col="red")  # add cutoff line
text(x=1:length(cooksd)+1, y=cooksd, labels=ifelse(cooksd>4*mean(cooksd), names(cooksd),""), col="red")  # add labels

pair_attr_bbs$cooksd <- cooksd
pair_attr_bbs$outlier <- ifelse(pair_attr_bbs$cooksd < 4*mean(cooksd), "keep","delete")

pair_attr_bbs2 <- pair_attr_bbs[pair_attr_bbs$outlier=="keep",]
pair_attr_bbs3 <- pair_attr_bbs[!(row.names(pair_attr_bbs) %in% x),]
dispersal <- unique(pair_attr_bbs[,c("spp", "species", "Breeding_AM")]) ## remove blackcap and willow warbler
length(unique(pair_attr_bbs$species))
pair_attr_bbs4 <- pair_attr_bbs[!(pair_attr_bbs$species=="Blackcap" | pair_attr_bbs$species=="Willow Warbler" |
                                  pair_attr_bbs$species=="Robin" | pair_attr_bbs$species=="Wren"),]
length(unique(pair_attr_bbs4$species))
pair_attr_bbs4 <- droplevels(pair_attr_bbs4)

# Removing Outliers
# influential row numbers
influential <- as.numeric(names(cooksd)[(cooksd > (4/sample_size))])
pair_attr_bbs2 <- pair_attr_bbs[influential, ]

############################################# PLOT REUSLT ############################################# 

## run model without mobility for each species and extract the slope (mid.year) coefficient for each
results_table<-NULL
for (i in unique(pair_attr_bbs4$spp)){
  print(i)
  ## if statement: if if the mixed effects model doesn't throw an error (levels of pairID), then run an lmer, else run a linear model 
  loadError=F
  a=try({mob_bbs <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + scale(renk_hab_sim) + scale(winter_rain) + 
                           scale(spring_rain) + scale(autumn_rain) + scale(mid.year) +
                              (1|pair.id), data=pair_attr_bbs4[pair_attr_bbs4$spp==i,])})
  loadError <- (is(a, 'try-error')|is(a,'error'))
  if(loadError==F){
    mob_bbs <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + scale(renk_hab_sim) + scale(winter_rain) + 
                      scale(spring_rain) + scale(autumn_rain) + scale(mid.year) + (1|pair.id), data=pair_attr_bbs4[pair_attr_bbs4$spp==i,])
  }else{
    mob_bbs <- lm(lag0 ~ scale(mean_northing) + scale(distance) + scale(renk_hab_sim) + scale(winter_rain) + 
                    scale(spring_rain) + scale(autumn_rain) + scale(mid.year),
                     data=pair_attr_bbs4[pair_attr_bbs4$spp==i,])
  }
...
## merge in this dataframe with specialism info (spec/gen)
mobility <- pair_attr_bbs4[,c("spp", "Breeding_AM")]
mobility <- unique(mobility)
results_table <- merge(results_table, mobility, by.x="i", by.y="spp")
## remove parameter column
results_table <- results_table[,-c(4)]
...
png("../Graphs/FINAL/FigureA3_birds.png", height = 11, width = 11, units = "cm", res = 300)
ggplot(mapping=aes(x=Mobility, y=slope, ymin=slope-SE, ymax=slope+SE)) +
  geom_point(data=results_table, size=2, color="lightgrey") +
  geom_errorbar(data=results_table, color="lightgrey", width=0.1) +
  geom_ribbon(data=model_summary, fill = "grey70", alpha=0.4) +
  geom_line(data=model_summary, size=1) +
  labs(x="Dispersal distance", y="Change in population synchrony 1999-2012") +
  scale_x_continuous(breaks = seq(0,4.5,0.5)) +
  theme_bw() +
