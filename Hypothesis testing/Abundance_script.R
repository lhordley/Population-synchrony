...
### put each species as either increasing, decreasing, or no change in abundance 
abundance_results_final <- abundance_results %>% group_by(Species.code) %>% mutate(abund_change = ifelse(p>0.05, "No change", ifelse(p<0.05 & mean_change_abund<0, "Decrease", "Increase")))
write.csv(abundance_results_final, file="../Results/Butterfly_results/abundance_results_85_00_3cat.csv", row.names=FALSE)
...
abundance_results <- read.csv("../Results/Butterfly_results/abundance_results_85_00_3cat.csv", header=TRUE)
...
## remove species with non-significant changes in abundance over time
abundance_results <- abundance_results[!abundance_results$abund_change=="No change",] ## 9 specices
...
abund_model1 <- lmer(lag0 ~ mean_northing + distance + renk_hab_sim + winter_rain + autumn_rain + spring_rain + summer_rain + 
                       winter_temp + autumn_temp + spring_temp + summer_temp + mid.year*abund_change + (1|spp) + (1|pair.id), data=pair_attr_ukbms)
summary(abund_model1)
anova(abund_model1)
## not significant (p=0.22)
## significant when looking at 9 spp with significant abund change
results_table_abund_ukbms <- data.frame(summary(abund_model1)$coefficients[,1:5]) ## 32 species // 9 species
write.csv(results_table_abund_ukbms, file = "../Results/Model_outputs/UKBMS/change_abund_85_00_ukbms_3cat.csv", row.names=TRUE)
...
abundance <- pair_attr_ukbms[,c(1,42)]
...
results_table_abund_ukbms$Abundance <- revalue(results_table_abund_ukbms$Abundance, c("mid.year:abund_changeIncrease"="Increase"))
...
png("../Graphs/Abundance/Abundance_change_ukbms_85_00_3cat.png", height = 150, width = 180, units = "mm", res = 300)
abund_85_00 <- ggplot(mapping=aes(x=Abundance, y=slope, ymin=slope-SE, ymax=slope+SE)) +
  ...
  labs(x="Change in abundance", y="Change in population synchrony 1985-2000") +
...
### put each species as either increasing, decreasing, or no change in abundance 
abundance_results_final <- abundance_results %>% group_by(Species.code) %>% mutate(abund_change = ifelse(p>0.05, "No change", ifelse(p<0.05 & mean_change_abund<0, "Decrease", "Increase")))
write.csv(abundance_results_final, file="../Results/Butterfly_results/abundance_results_00_12_3cat.csv", row.names=FALSE)

## save abundance results
write.csv(abundance_results, file="../Results/Butterfly_results/abundance_results_00_12.csv", row.names=FALSE)
abundance_results <- read.csv("../Results/Butterfly_results/abundance_results_00_12_3cat.csv", header=TRUE)
...
## remove species with non-significant changes in abundance over time
abundance_results <- abundance_results[!abundance_results$abund_change=="No change",] ## 10 specices
## merge with abundance data 
pair_attr_ukbms <- merge(pair_attr_ukbms, abundance_results, by.x="spp", by.y="Species.code", all=FALSE)
pair_attr_ukbms <- droplevels(pair_attr_ukbms)
...
## still significant with 10 spp
results_table_abund_ukbms <- data.frame(summary(abund_model2)$coefficients[,1:5]) ## 32 species // 10 species
write.csv(results_table_abund_ukbms, file = "../Results/Model_outputs/UKBMS/change_abund_00_12_ukbms_3cat.csv", row.names=TRUE)
...
png("../Graphs/Abundance/Abundance_change_ukbms_00_12_3cat.png", height = 150, width = 180, units = "mm", res = 300)
...
## plot together
library(ggpubr)
png("../Graphs/FINAL/FigureA3.png", height = 200, width = 150, units = "mm", res = 300)
ggarrange(abund_85_00, abund_00_12, 
...
### put each species as either increasing, decreasing, or no change in abundance 
abundance_results_cbc_final <- abundance_results_cbc %>% group_by(species_code) %>% mutate(abund_change = ifelse(p>0.05, "No change", ifelse(p<0.05 & mean_abund_change<0, "Decrease", "Increase")))
write.csv(abundance_results_cbc_final, file="../Results/Bird_results/abundance_results_85_96_3cat.csv", row.names=FALSE)

## save abundance results
write.csv(abundance_results_cbc, file="../Results/Bird_results/abundance_results_85_96.csv", row.names=FALSE)
abundance_results_cbc <- read.csv("../Results/Bird_results/abundance_results_85_96_3cat.csv", header=TRUE)
...
## remove species with non-significant changes in abundance over time
abundance_results_cbc <- abundance_results_cbc[!abundance_results_cbc$abund_change=="No change",] ## 20 specices (removes 3)
## merge abundance data
pair_attr_cbc <- merge(pair_attr_cbc, abundance_results_cbc, by.x="spp", by.y="species_code", all=FALSE)
length(unique(pair_attr_cbc$spp)) # 19 species 
#pair_attr_cbc <- pair_attr_cbc[-c(21:25)]

###### run model with strategy and year interaction
abund_model_cbc <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + hab_sim + scale(summer_temp) + scale(mid.year)*abund_change + (1|family) + (1|pair.id) + (1|spp), data = pair_attr_cbc)
summary(abund_model_cbc) ## interaction is non-significant (p=0.0601)
## significant (0.0493) when using only significant changes in abundance 
...
for (i in unique(pair_attr_cbc$spp)){
  ...
  # if statement: if if the mixed effects model doesn't throw an error (levels of pairID), then run an lmer, else run a linear model
  loadError=F
  a=try({spec_ukbms <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + hab_sim + scale(mid.year) + scale(summer_temp) + (1|pair.id), data=pair_attr_cbc[pair_attr_cbc$spp==i,])})
  loadError <- (is(a, 'try-error')|is(a,'error'))
  if(loadError==F){
    spec_ukbms <- lmer(lag0 ~ scale(mean_northing) + scale(distance) + hab_sim + scale(mid.year) + scale(summer_temp) + (1|pair.id), data=pair_attr_cbc[pair_attr_cbc$spp==i,])
  }else{
    spec_ukbms <- lm(lag0 ~  scale(mean_northing) + scale(distance) + hab_sim + scale(mid.year) + scale(summer_temp), data=pair_attr_cbc[pair_attr_cbc$spp==i,])
  }
...
## merge in this dataframe with abundance info 
abundance <- pair_attr_cbc[,c(1,43)]
abundance <- unique(abundance)
results_table <- merge(results_table, abundance, by.x="i", by.y="spp")
## remove parameter column
results_table <- results_table[,-c(4)]
...
results_table_abund_cbc$Abundance <- revalue(results_table_abund_cbc$Abundance, c("scale(mid.year):abund_changeIncrease"="Increase"))
...
abund <- ggplot(mapping=aes(x=Abundance, y=slope, ymin=slope-SE, ymax=slope+SE)) +
  ...
  geom_point(data=model_summary, size=4) +
  geom_errorbar(data=model_summary, width=0.1) +
  labs(x="Change in abundance", y="Change in population synchrony 1985-1996") +
  #scale_y_continuous(breaks = seq(-0.02,0.03,0.01)) +
  theme_bw() +
...
abund
dev.off()
## increasing in abund slightly greater change in synchrony (only just significant)
dat <- ggpredict(abund_model_cbc, terms = c("mid.year", "abund_change"))
plot(dat) # higher mobility increase in synchrony more steeply than low mobility
...
### put each species as either increasing, decreasing, or no change in abundance 
abundance_results_bbs_final <- abundance_results_bbs %>% group_by(species_code) %>% mutate(abund_change = ifelse(p>0.05, "No change", ifelse(p<0.05 & mean_abund_change<0, "Decrease", "Increase")))
write.csv(abundance_results_bbs_final, file="../Results/Bird_results/abundance_results_99_12_3cat.csv", row.names=FALSE)

## save abundance results
write.csv(abundance_results_bbs, file="../Results/Bird_results/abundance_results_99_12.csv", row.names=FALSE)
abundance_results_bbs <- read.csv("../Results/Bird_results/abundance_results_99_12_3cat.csv", header=TRUE)
...
## remove species with non-significant changes in abundance over time
abundance_results_bbs <- abundance_results_bbs[!abundance_results_bbs$abund_change=="No change",] ## 17 specices
## merge with abundance data
pair_attr_bbs <- merge(pair_attr_bbs, abundance_results_bbs, by.x="spp", by.y="species_code", all=FALSE)
length(unique(pair_attr_bbs$spp)) ## 12 species
...
abund_model_bbs <- lmer(lag0 ~ mean_northing + distance + renk_hab_sim + winter_rain + autumn_rain + spring_rain + 
                          mid.year*abund_change + (1|pair.id) + (1|spp), data = pair_attr_bbs)
